( // 闭包
    function(a,b){ // a:window b:document
        ...
        var song_data = null;
        // ajax获取数据
        function data_transfer(method,url,flag,callback){
            ...
            function(res){
                ...
                var Player = {
                    ...
                    init: function(){
                        ...
                        v_name.innerHTML = song_data[0].title;
                        v_author.innerHTML = song_data[0].author;
                        m_link.setAttribute("data-audioid",song_data[0].id) 
                        img_box.style.backgroundImage = "url(" + song_data[0].img + ")";
                        m.addEventListener("canplay",this.msg_init,false)
                        // 初始音量
                        m.volume = 0.3;
                        // 初始音量条进度
                    ...
                    menu_init: function(){
                        ...
                        for(var i = 0; i < song_data.length; i++){
                            munu_list_html += 
                                "<li class='style-list-link' data-url='" 
                                + song_data[i].href 
                                + "' data-audioid='" 
                                + song_data[i].id 
                                + "'>" + song_data[i].title 
                        ...
                        music_list.innerHTML = munu_list_html;
                    },
                    // 播放暂停
                    switchPlay: function(){
                    ...
                    prevFn: function(){
                        var prev = m_link.getAttribute("data-audioid");
                        for(var i = 0; i < list_link.length; i++){
                            if(prev == song_data[i].id){
                                var j = i - 1;
                                if(j < 0){
                                    j = list_link.length - 1;
                                }
                                // 设置当前播放的音乐id
                                m_link.setAttribute("data-audioid",song_data[j].id)
                                console.log(j)
                                // 设置当前播放的音乐链接
                                m_link.src = song_data[j].href;
                                // 加载&播放
                                m.load()
                                m.autoplay = true;
                                // 播放按钮隐藏
                                ...
                                console.log(m_link.src)
                                // 设置播放信息
                                v_name.innerHTML = song_data[j].title;
                                v_author.innerHTML = song_data[j].author;
                                img_box.style.backgroundImage = "url(" + song_data[j].img + ")"; 
                                break;
                            }
                        }
                    },
                    ...
                    nextFn: function(){
                        var next = m_link.getAttribute("data-audioid");
                        console.log(next)
                        for(var i = 0; i < list_link.length; i++){
                            // console.log(list_link[i].getAttribute("data-url"))
                            if(next == song_data[i].id){
                                var j = i + 1;
                    ...
                    link_switch: function(){
                        var link_id = this.getAttribute("data-audioid"); // 获取当前点击的id
                        for(var i = 0; i < list_link.length; i++){
                            if(link_id == song_data[i].id){ // 如果点击的id等于当前音乐的id
                                m_link.setAttribute("data-audioid",song_data[i].id) // 设置当前播放的音乐id
                                m_link.src = song_data[i].href; // 设置当前播放的音乐链接
                                m.load()
                                m.autoplay = true;
                                play_btn.style.display = "none";
                                pause_btn.style.display = "block";
                                console.log(m_link.src)
                                v_name.innerHTML = song_data[i].title;
                                v_author.innerHTML = song_data[i].author;
                                img_box.style.backgroundImage = "url(" + song_data[i].img + ")"; 
                                break;
                            }
                        }
                    },
